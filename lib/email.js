...
const ADMIN_EMAIL = 'Edenprojectcompany@gmail.com';
...
function orderConfirmationHtml({ ref, name, amount, items, provider }) {
  const itemRows = (items || []).map(i =>
    `<tr><td style="padding:6px 0;color:#c0c0c8">${i.name || i}</td><td style="padding:6px 0;text-align:right;color:#f4f4f6">${i.qty || ''} × ${i.unitPrice || ''}€</td></tr>`
  ).join('');
...
<div style="max-width:540px;margin:0 auto;background:#111114;border-radius:20px;border:1px solid rgba(255,255,255,.08);overflow:hidden">
  ...
  <div style="padding:32px 36px">
    ...
    <p style="color:#8a8a93;font-size:14px;margin:0 0 24px">Bonjour ${name || 'Collectionneur'}, votre paiement a bien été reçu.</p>
...
<div style="max-width:500px;background:#111114;border-radius:16px;padding:24px;border:1px solid #333">
  ...
  <table style="width:100%;border-collapse:collapse">
    <tr><td style="color:#8a8a93;padding:5px 0">Référence</td><td style="color:#f4f4f6;font-weight:700">${ref}</td></tr>
    <tr><td style="color:#8a8a93;padding:5px 0">Client</td><td style="color:#f4f4f6">${customerName||'—'} &lt;${customerEmail||'—'}&gt;</td></tr>
    ...
    ${promoCode?`<tr><td style="color:#8a8a93;padding:5px 0">Code promo</td><td style="color:#c9a8ff">${promoCode}</td></tr>`:''}
    ${prizeCode?`<tr><td style="color:#8a8a93;padding:5px 0">⚠️ Prix roue</td><td style="color:#ff7a7a;font-weight:700">${prizeCode} — FULFILLMENT REQUIS</td></tr>`:''}
  </table>
</div>
</body></html>`;
}
...
<div style="max-width:480px;margin:0 auto;background:#111114;border-radius:20px;border:1px solid rgba(255,255,255,.08);overflow:hidden">
  ...
  <div style="padding:32px 36px">
    ...
    <p style="color:#8a8a93;font-size:14px;margin:0 0 24px">Bonjour ${name||'Collectionneur'},<br>Cliquez sur le bouton ci-dessous pour définir un nouveau mot de passe. Ce lien est valable <strong style="color:#f4f4f6">1 heure</strong>.</p>
