...
function orderConfirmationHtml({ ref, name, amount, items, provider }) {
  const itemRows = (items || []).map(i =>
    `<tr><td style="padding:6px 0;color:#c0c0c8">${escHtml(i.name || i)}</td><td style="padding:6px 0;text-align:right;color:#f4f4f6">${escHtml(i.qty || '')} × ${escHtml(i.unitPrice || '')}€</td></tr>`
  ).join('');
